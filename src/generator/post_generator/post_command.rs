...
pub fn handle_post_generate_command(
    ...
    match val {
        Value::String(_) | Value::Array(_) => execute_commands(val, project_dir, template_data)?,
        Value::Object(ref commands_map) => {
            if let Some(val) = commands_map.get(constants::TEMPLATE_OS_FLAG_ALL) {
                execute_commands(val, project_dir, template_data)?;
            }
...
fn execute_commands(val: &Value, project_dir: &Path, template_data: &HashMap<&str, Value>) -> crate::Result<()> {
    match val {
        Value::String(ref command) => execute_single_command(command.as_str(), project_dir, template_data)?,
        Value::Array(ref commands) => {
            for command in commands.iter().filter_map(|v| v.as_str()) {
                execute_single_command(command, project_dir, template_data)?
            }
        }
        _ => (),
    }
...
fn execute_single_command(
    ...
    let envs = template_data
        ...
        .chain(gen_extra_template_data(project_dir));
